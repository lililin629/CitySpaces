...
#TODO: Google Maps Nearby Search Place_type: tourist_attraction to replace sights returned by chatGPT
...
def call_chat(city_name, params):
    sights = nearby.get_sights(city_name)
    response = openai.ChatCompletion.create(
        model="gpt-3.5-turbo",
        messages=[
            {"role": "user", "content": "This is an example json:" + json.dumps(params)},
            {"role": "user", "content" : "Please write a similar json for " + city_name +  ", with the following sights: " + f"sight1: {sights[0]}, sight2: {sights[1]}, sight3: {sights[2]}."}])
